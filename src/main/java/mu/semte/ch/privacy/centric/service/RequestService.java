...
public class RequestService {
  ...
  public void processUpdate(String payload, String sessionId) {
    ...
    var updateRequest = Optional.ofNullable(document.get()).orElseThrow(()-> new RuntimeException("could not parse request"));
    ...
    if(!parameters.isEmpty()){
      ...
      parameters.put("time", ModelUtils.formattedDate(LocalDateTime.now()));
  ...
  public String processRead(String payload, String sessionId){
    var pirb = PersonInformationRequest.builder();
    ...
    var readRequest = Optional.ofNullable(document.get()).orElseThrow(()-> new RuntimeException("could not parse request"));
    ...
    if(!res.isEmpty()){
      ...
      pirb.dateOfBirth(mapRes.get("dateOfBirth"));
      pirb.registrationNumber(mapRes.get("registrationNumber"));
      String genderId = mapRes.get("genderId");
      if(isNotEmpty(genderId)){
        pirb.gender(Gender.builder().id(genderId).build());
      }
    }

    String getPersonNationalities = queryStore.getQueryWithParameters("getPersonNationalities", parameters);//nationaliteit
    ...
    if(!nationaliteits.isEmpty()){
      pirb.nationalities(nationaliteits.stream().map(n -> n.get("nationaliteitId")).filter(StringUtils::isNotEmpty).map(n -> Nationality.builder().id(n).build()).collect(Collectors.toList()));
    }
    pirb.reason(readRequest.getReason());
    ...
    log.info(requestReadReason);
    sparqlClient.executeUpdateQuery(requestReadReason);

    byte[] responseBytes = resourceConverter.writeDocument(new JSONAPIDocument<>(pirb.build()));
